  ...
  setup(__props) {
    ...
    const markers = common_vendor.computed(() => {
      return rideStore.rides.map((r) => ({
        // 优先使用云端 _id
        id: r._id || r.id,
        // 安全地从 start_location 读取经纬度，如果没有就降级使用原来的 lat/lng
        latitude: r.start_location ? r.start_location.lat : r.lat,
        longitude: r.start_location ? r.start_location.lng : r.lng,
        title: r.driver,
    ...
    return (_ctx, _cache) => {
      return {
        ...
        n: common_vendor.o(goPublish, "1a")
      };
    };
  }
};
